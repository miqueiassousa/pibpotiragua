<!DOCTYPE html>
<html lang="pt-br">

<head>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8">
    <title>PIB Potiraguá</title>

    <!-- Required meta tags -->
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">

    <!-- Bootstrap CSS -->
    <link rel="stylesheet" href="css/owl/owl.carousel.min.css" />
    <link rel="stylesheet" href="css/owl/owl.theme.default.min.css" />
    <link rel="stylesheet" href="css/magnific-popup.css" />
    <link rel="stylesheet" href="css/main.css" />

    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.3/css/all.min.css" />
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/OwlCarousel2/2.3.4/assets/owl.carousel.min.css" />

    <!-- SCRIPT -->

    <script src="https://cdnjs.cloudflare.com/ajax/libs/typed.js/2.0.11/typed.min.js"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.1.1/js/all.min.js"></script>


    <script src="https://code.jquery.com/jquery-3.5.1.min.js"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/waypoints/4.0.1/jquery.waypoints.min.js"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/OwlCarousel2/2.3.4/owl.carousel.min.js"></script>


</head>



<?php

/*
require 'config.php';
require 'dao/EventoDaoMySql.php';

ini_set('default_charset', 'utf-8');

$eventoDao = new EventoDaoMysql($pdo);
$lista = $eventoDao->findAll();

*/
?>


<div class="menu-geral">
    <div id="header">
        <!-- <div class="container"> -->
        <div class="container">
            <nav class="navbar navbar-expand-lg navbar-light justify-content-between">

                <!-- <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarNav" aria-controls="navbarNav" aria-expanded="false" aria-label="Toggle navigation">
                            <span class="navbar-toggler-icon"></span>
                        </button> -->
                <!-- <div class="collapse navbar-collapse" id="navbarNav">-->
                <div>
                    <ul class="menu">
                        <li><a class="menu-btn active" href="#">Home <span class="sr-only">(current)</span></a></li>
                        <li><a class="menu-btn" href="#galeria">Galeria</a></li>
                        <li><a class="menu-btn" href="#sobre">Sobre</a></li>
                        <li><a class="menu-btn" href="#contato">Contato</a></li>
                    </ul>

                </div>

                <div class="redesocial-menu">
                    <ul class="list">
                        <li>
                            <a href="https://www.facebook.com/pibjerusalempotiragua" target="_blank" rel="noopener noreferrer"><i class="redesocial fa-brands fa-facebook"></i></a>
                        </li>

                        <li>
                            <a href="https://www.instagram.com/juventudepibj/" target="_blank" rel="noopener noreferrer"> <i class="redesocial fa-brands fa-instagram"></i></i></a>
                        </li>
                    </ul>

                </div>
                <div class="menu-btn menu-icone">
                    <i class="fas fa-bars"></i>
                </div>
            </nav>
        </div>

        <!-- </div> -->
    </div> <!-- Fim da Header -->

    <section class="conteudo-header">
        <div class="container">
            <div class="justify-content-between flex row">
                <div class="conteudo-1 col-lg-8 col-md-12 col-sm-12 mb-0 mt-0 mb-lg-0">
                    <h1 class="titulo1">"Ide por todo o mundo, pregai o evangelho a toda criatura."</h1>

                    <p class="green titulo2">Quarta Feira 19:30 | Domingo 19:30</p>
                </div>
                <div class="conteudo-2 col-lg-4 col-md-12 col-sm-12 mb-0 mt-0 mb-lg-0">
                    <div class="proximo">
                        <p class="titulo2">PRÓXIMOS EVENTOS</p>
                    </div>
                    <div class="eventos">



                        <div class="evento">
                            <div class="data">
                                <div class="mes">
                                    <p>MAI</p>
                                </div>
                                <div class="numero">
                                    <p>05</p>
                                </div>
                            </div>
                            <div class="traco"></div>
                            <div class="descricao">
                                <p>PIB Potiragua</p>
                                <div class="onde">
                                    <p class="bold">Posso do Pr. Biafra</p>
                                </div>
                                <p>Sab 19:00</p>
                            </div>
                        </div>



                    </div>

                </div>
            </div>
        </div>
    </section>
</div> <!-- Menu Geral -->


<div class="section">
    <div class="container">
        <div id="versiculo">
            <h1 class="titulo">Querido Deus, Tu és minha proteção, a minha fortaleza. <br />Tu és o meu Deus, eu
                confio em Ti.</h1>
            <p class="texto lato opaco">(Salmo 91:2)</p>
        </div>

    </div>
</div>
<div class="container">

</div>

<div id="sobre">
    <div class="container">
        <div class="sobre-texto row flex">
            <div class="img-igreja col-lg-6 col-sm-12 col-md-12">
                <div>
                    <img src="assets/images/antesedepoisigreja.png" />
                </div>
            </div>
            <div class="conteudo-sobre col-lg-6 col-sm-12 col-md-12">
                <h1 class="titulo1 center">SOBRE</h1>
                <p class="lato texto">Juntos formamos uma comunidade evangélica desde os anos 80 como congregação
                    da Primeira Igreja Batista de Itarantim - BA, tendo como pastor Francisco Vasconcelos Xavier
                    que descobriu a existência de duas familiais remanescentes residentes na cidade de Potiraguá,
                    a família de José Alves de Oliveira e Osório Francisco Pereira, começaram se reunir no prédio
                    do antigo cinema na Rua Adhemar Menezes, onde deu inicio a toda história até ser emancipada
                    como Primeira Igreja Batista em Potiragua na Praça Rita Maria Alves, em frente a prefeitura
                    municipal. <br /><br />Neste período muitos pastores e missionários marcaram a história
                    principalmente o
                    emérito Pr. Paulino Peixoto dos Santos e sua esposa Edna Peixoto dos Santos, ao longo de 16
                    anos como pastor titular, tempo esse onde muitas vidas foram alcançadas através do evangelho
                    de Cristo e seus ensinamentos Bíblicos. <br /><br />Antes da sua emancipação a Primeira Igreja
                    Batista
                    em Potiraguá também pertenceu a Igreja Batista Jerusalém em Salvador – BA na direção do pastor
                    emérito Carlos Antônio dos Santos. Atualmente temos como pastor presidente o
                    Pr. Biafra Santos. </p>

            </div>
        </div>
    </div>

</div>

<!--NOSSOS SERVICOS-->
<div id="servicos" class="block">
    <div class="container">

        <div class="row center">
            <div class="col-lg-3 col-md-6 mb-lg-0 mb-4">
                <a href="#" class="destaque">
                    <div class="imagem circle">
                        <img src="assets/galeria/pastorbiafra1.jpg" class="img-fluid" />
                    </div>
                    <p>Pr. Biafra Santos</p>
                    <p class="subtitulo">
                        Pastor Presidente
                    </p>
                </a>
            </div>
            <div class="col-lg-3 col-md-6 mb-lg-0 mb-4">
                <a href="#" class="destaque">
                    <div class="imagem circle">
                        <img src="assets/galeria/carolinaferraz.jpg" class="img-fluid" />
                    </div>
                    <p>Carolina Ferraz</p>
                    <p class="subtitulo">
                        Ministra de Louvor
                    </p>
                </a>
            </div>

        </div>
    </div>
</div>
<!--//NOSSOS SERVICOS-->

<!--galeria-->
<div id="galeria" class="block">
    <div class="container">

        <div class="galeria-grupo">

            <div class="button-group">
                <button type="button" class="active text-button" data-filter="*" id="btn-all">
                    Todos
                </button>
                <button type="button" class="text-button" data-filter=".igreja">IGREJA</button>
                <button type="button" class="text-button" data-filter=".historia">HISTÓRIA</button>
                <button type="button" class="text-button" data-filter=".juventude">JUVENTUDE</button>
                <button type="button" class="text-button" data-filter=".mcm">MCM</button>
                <button type="button" class="text-button" data-filter=".criancas">Crianças</button>
                <button type="button" class="text-button" data-filter=".mensageiras">Mensageiras</button>
                <button type="button" class="text-button" data-filter=".embaixadores">Embaixadores</button>
                <button type="button" class="text-button" data-filter=".recente">Recentes</button>

            </div>

            <div class="row grid">

                <div class="col-lg-4 col-6 mb-4 element-item igreja">
                    <a href="assets/galeria/pastorbiafra1.jpg" class="popup-link">
                        <img src="assets/galeria/pastorbiafra1.jpg" class="img-fluid mb-2" />
                    </a>
                    <p class="titulo2 mb-1">Pr. Biafra Santos</p>

                </div>

                <div class="col-lg-4 col-6 mb-4 element-item igreja">
                    <a href="assets/galeria/pastorbiafra2.jpg" class="popup-link">
                        <img src="assets/galeria/pastorbiafra2.jpg" class="img-fluid mb-2" />
                    </a>
                    <p class="titulo2 mb-1">Pr. Biafra Santos e Carol</p>

                </div>

                <div class="col-lg-4 col-6 mb-4 element-item igreja">
                    <a href="assets/galeria/pastorbiafra3.PNG" class="popup-link">
                        <img src="assets/galeria/pastorbiafra3.PNG" class="img-fluid mb-2" />
                    </a>
                    <p class="titulo2 mb-1">Pr. Biafra Santos e Carol</p>

                </div>

                <div class="col-lg-4 col-6 mb-4 element-item historia">
                    <a href="assets/galeria/prpaulino.jpg" class="popup-link">
                        <img src="assets/galeria/prpaulino.jpg" class="img-fluid mb-2" />
                    </a>
                    <p class="titulo2 mb-1">Pr. Paulino e Edna Peixoto </p>

                </div>

                <div class="col-lg-4 col-6 mb-4 element-item historia">
                    <a href="assets/galeria/prpaulino4.jpg" class="popup-link">
                        <img src="assets/galeria/prpaulino4.jpg" class="img-fluid mb-2" />
                    </a>
                    <p class="titulo2 mb-1">Pr. Paulino e família</p>

                </div>

                <div class="col-lg-4 col-6 mb-4 element-item historia">
                    <a href="assets/galeria/prcarlos.png" class="popup-link">
                        <img src="assets/galeria/prcarlos.png" class="img-fluid mb-2" />
                    </a>
                    <p class="titulo2 mb-1">Pr. Carlos Antônio </p>

                </div>

                <div class="col-lg-4 col-6 mb-4 element-item historia">
                    <a href="assets/galeria/igrejaantes.jpg" class="popup-link">
                        <img src="assets/galeria/igrejaantes.jpg" class="img-fluid mb-2" />
                    </a>
                    <p class="titulo2 mb-1">1ª Igreja Batista (Jerusalem) </p>

                </div>

                <div class="col-lg-4 col-6 mb-4 element-item historia">
                    <a href="assets/galeria/prpaulino2.jpg" class="popup-link">
                        <img src="assets/galeria/prpaulino2.jpg" class="img-fluid mb-2" />
                    </a>
                    <p class="titulo2 mb-1">Pr Paulino Peixoto </p>

                </div>


                <div class="col-lg-4 col-6 mb-4 element-item mensageiras embaixadores">
                    <a href="assets/galeria/mensageiras.jpg" class="popup-link">
                        <img src="assets/galeria/mensageiras.jpg" class="img-fluid mb-2" />
                    </a>
                    <p class="titulo2 mb-1">Pr Paulino Peixoto </p>

                </div>

                <div class="col-lg-4 col-6 mb-4 element-item mensageiras">
                    <a href="assets/galeria/mensageiras2.jpg" class="popup-link">
                        <img src="assets/galeria/mensageiras2.jpg" class="img-fluid mb-2" />
                    </a>
                    <p class="titulo2 mb-1">Pr Paulino Peixoto </p>

                </div>

                <div class="col-lg-4 col-6 mb-4 element-item igreja">
                    <a href="assets/galeria/igreja1.jpg" class="popup-link">
                        <img src="assets/galeria/igreja1.jpg" class="img-fluid mb-2" />
                    </a>
                    <p class="titulo2 mb-1">Batismo</p>

                </div>

                <div class="col-lg-4 col-6 mb-4 element-item igreja">
                    <a href="assets/galeria/igreja2.jpg" class="popup-link">
                        <img src="assets/galeria/igreja2.jpg" class="img-fluid mb-2" />
                    </a>
                    <p class="titulo2">PIB em Potiraguá</p>

                </div>

                <div class="col-lg-4 col-6 mb-4 element-item igreja">
                    <a href="assets/galeria/igreja3.jpg" class="popup-link">
                        <img src="assets/galeria/igreja3.jpg" class="img-fluid mb-2" />
                    </a>
                    <p class="titulo2">PIB em Potiraguá</p>

                </div>

                <div class="col-lg-4 col-6 mb-4 element-item igreja">
                    <a href="assets/galeria/igreja5.jpg" class="popup-link">
                        <img src="assets/galeria/igreja5.jpg" class="img-fluid mb-2" />
                    </a>
                    <p class="titulo2">Batismo</p>

                </div>

                <!-- JOVENS -->

                <div class="col-lg-4 col-6 mb-4 element-item juventude">
                    <a href="assets/galeria/jovens1.jpg" class="popup-link">
                        <img src="assets/galeria/jovens1.jpg" class="img-fluid mb-2" />
                    </a>
                    <p class="titulo2">Juventude</p>

                </div>


                <div class="col-lg-4 col-6 mb-4 element-item juventude">
                    <a href="assets/galeria/jovens3.JPG" class="popup-link">
                        <img src="assets/galeria/jovens3.JPG" class="img-fluid mb-2" />
                    </a>
                    <p class="titulo2">Juventude</p>

                </div>

                <div class="col-lg-4 col-6 mb-4 element-item juventude">
                    <a href="assets/galeria/jovens4.JPG" class="popup-link">
                        <img src="assets/galeria/jovens4.JPG" class="img-fluid mb-2" />
                    </a>
                    <p class="titulo2">Juventude</p>

                </div>

                <div class="col-lg-4 col-6 mb-4 element-item juventude">
                    <a href="assets/galeria/jovens5.jpg" class="popup-link">
                        <img src="assets/galeria/jovens5.jpg" class="img-fluid mb-2" />
                    </a>
                    <p class="titulo2">Teatro</p>

                </div>

                <div class="col-lg-4 col-6 mb-4 element-item juventude">
                    <a href="assets/galeria/jovens6.jpg" class="popup-link">
                        <img src="assets/galeria/jovens6.jpg" class="img-fluid mb-2" />
                    </a>
                    <p class="titulo2">Juventude</p>

                </div>

                <div class="col-lg-4 col-6 mb-4 element-item juventude">
                    <a href="assets/galeria/jovens7.jpg" class="popup-link">
                        <img src="assets/galeria/jovens7.jpg" class="img-fluid mb-2" />
                    </a>
                    <p class="titulo2">Juventude</p>

                </div>

                <div class="col-lg-4 col-6 mb-4 element-item juventude">
                    <a href="assets/galeria/jovens8.jpg" class="popup-link">
                        <img src="assets/galeria/jovens8.jpg" class="img-fluid mb-2" />
                    </a>
                    <p class="titulo2">Juventude</p>

                </div>

                <div class="col-lg-4 col-6 mb-4 element-item recente juventude">
                    <a href="assets/galeria/jovens10.jpg" class="popup-link">
                        <img src="assets/galeria/jovens10.jpg" class="img-fluid mb-2" />
                    </a>
                    <p class="titulo2">Juventude</p>

                </div>

                <!-- MCA -->

                <div class="col-lg-4 col-6 mb-4 element-item mcm">
                    <a href="assets/galeria/mca5.jpg" class="popup-link">
                        <img src="assets/galeria/mca5.jpg" class="img-fluid mb-2" />
                    </a>
                    <p class="titulo2">Mulheres</p>

                </div>

                <div class="col-lg-4 col-6 mb-4 element-item mcm">
                    <a href="assets/galeria/mca2.jpg" class="popup-link">
                        <img src="assets/galeria/mca2.jpg" class="img-fluid mb-2" />
                    </a>
                    <p class="titulo2">Mulheres</p>

                </div>

                <div class="col-lg-4 col-6 mb-4 element-item mcm">
                    <a href="assets/galeria/mca3.jpg" class="popup-link">
                        <img src="assets/galeria/mca3.jpg" class="img-fluid mb-2" />
                    </a>
                    <p class="titulo2">Mulheres</p>

                </div>

                <div class="col-lg-4 col-6 mb-4 element-item mcm">
                    <a href="assets/galeria/mca4.jpg" class="popup-link">
                        <img src="assets/galeria/mca4.jpg" class="img-fluid mb-2" />
                    </a>
                    <p class="titulo2">Mulheres</p>

                </div>

                <!-- Crianças -->

                <div class="col-lg-4 col-6 mb-4 element-item criancas">
                    <a href="assets/galeria/crianças2.jpg" class="popup-link">
                        <img src="assets/galeria/crianças2.jpg" class="img-fluid mb-2" />
                    </a>
                    <p class="titulo2">Mulheres</p>

                </div>

                <div class="col-lg-4 col-6 mb-4 element-item criancas">
                    <a href="assets/galeria/crianças1.jpg" class="popup-link">
                        <img src="assets/galeria/crianças1.jpg" class="img-fluid mb-2" />
                    </a>
                    <p class="titulo2">Mulheres</p>

                </div>

            </div>
        </div>
    </div>
</div>
<!--//galeria-->

<div id="jovens" class="jovens">
    <div class="container">
        <div class="sobre-texto row flex">
            <div class="img-igreja col-lg-6 col-sm-12 col-md-12">
                <div>
                    <img src="assets/images/jovens-foto.png" />
                </div>
            </div>
            <div class="conteudo-sobre col-lg-6 col-sm-12 col-md-12">
                <h1 class="lato bold">JUVENTUDE</h1>
                <p class="lato texto"> Alegre-se, jovem, na sua mocidade! Seja feliz o seu coração nos dias
                    da sua juventude! Siga por onde seu coração mandar, até onde a sua vista alcançar; mas
                    saiba que por todas essas coisas Deus o trará a julgamento.<br />
                    Eclesiastes 11:9<br /><br />
                    Da mesma forma, jovens, sujeitem-se aos mais velhos. Sejam todos humildes uns para com os
                    outros, porque "Deus se opõe aos orgulhosos, mas concede graça aos humildes". Portanto,
                    humilhem-se debaixo da poderosa mão de Deus, para que ele os exalte no tempo devido.<br />
                    1 Pedro 5:5-6 <br /><br />
                    Somente seja forte e muito corajoso! Tenha o cuidado de obedecer a toda a lei que o meu
                    servo Moisés lhe ordenou; não se desvie dela, nem para a direita nem para a esquerda,
                    para que você seja bem sucedido por onde quer que andar.<br />
                    Josué 1:7<br /><br />
                    Pois tu és a minha esperança, ó Soberano Senhor, em ti está a minha confiança desde a
                    juventude.<br />
                    Salmos 71:5
                </p>
                <li class="lato icone">
                    <a href="https://www.instagram.com/juventudepibj/" target="_blank" rel="noopener noreferrer"> <i class="fa-brands fa-instagram"> </i></i>juventudepibj</a>
                </li>


            </div>
        </div>
    </div>
</div> <!-- Jovens -->



<div class="contato" id="contato">
    <div class="container">
        <div class="colunas row">
            <div class="coluna1 col-lg-2 col-md-2 col-sm-12">
                <img src="assets/images/logo.png" />

            </div>

            <div class="coluna2 col-lg-2 col-md-2 col-sm-12 ml-0">
                <div class="column">
                    <p class="titulo">LINKS</p>
                    <div class="menu-contato">
                        <ul>
                            <li><a href="#">Home <span class="sr-only">(current)</span></a></li>
                            <li><a href="#galeria">Programação</a></li>
                            <li><a href="#sobre">Sobre</a></li>
                            <div class="redes-social">
                                <li>
                                    <a href="https://www.facebook.com/pibjerusalempotiragua" target="_blank" rel="noopener noreferrer"><i class="redesocial fa-brands fa-facebook"></i></a>
                                </li>

                                <li>
                                    <a href="https://www.instagram.com/juventudepibj/" target="_blank" rel="noopener noreferrer"> <i class="redesocial fa-brands fa-instagram"></i></i></a>
                                </li>
                            </div>
                        </ul>
                    </div>
                </div>
            </div>

            <div class="linha-vertical col-lg-1 col-md-1 col-sm-12 ml-0"></div>

            <div class="coluna3 col-lg-2 col-md-3 col-sm-12 ml-0">
                <div class="column">
                    <p class="titulo">ENDEREÇO</p>
                    <p class="opaco">Praça Getúlio Vargas</p>
                    <p class="opaco">Centro - Potiraguá/BA </p>
                    <p class="opaco">CEP: 45790-000</p>
                </div>
            </div>

            <div class="linha-vertical col-lg-1 col-md-1 col-sm-12 ml-0"></div>

            <div class="coluna4 col-lg-2 col-md-3 col-sm-12 ml-0">
                <div class="column">
                    <p class="titulo">CONTRIBUA</p>
                    <p class="opaco">Bradesco</p>
                    <p class="opaco">Ab: 3537</p>
                    <p class="opaco">Conta: 500516-7</p>
                </div>
            </div>

            <div class="linha-vertical col-lg-0 col-md-1 col-sm-12 ml-0"></div>

            <div class="coluna5 col-lg-1 col-md-2 col-sm-12 ml-0">
                <div class="column">
                    <p class="titulo">CONTATO</p>
                    <p class="opaco">(73) 00000-0000</p>
                    <p class="opaco">contato@pibpotiragua.com.br</p>
                </div>
            </div>
        </div>
    </div>
</div>

<div class="rodape">
    <div class="footer">

        <span class="texto">Created By <a href="https://www.miqueiassousa.com">Miquéias Sousa</a> | <span class="far fa-copyright"></span>
            2022 All rights reserved.
        </span>

    </div>
</div>

<!--JS-->
<script src="https://cdn.jsdelivr.net/npm/popper.js@1.16.0/dist/umd/popper.min.js" integrity="sha384-Q6E9RHvbIyZFJoft+2mJbHaEWldlvI9IOYy5n3zV9zzTtmI3UksdQRVvoxMfooAo" crossorigin="anonymous"></script>
<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script><!-- Jquery ok -->
<script src="js/bootstrap.min.js"></script> <!-- Bootstrap -->
<script src="js/owl.carousel.min.js"></script> <!-- Depoimentos -->
<script src="js/isotope.pkgd.min.js"></script> <!-- Galeria de fotos ok-->
<script src="js/magnify/jquery.magnific-popup.min.js"></script> <!-- Pasagem das fotos ok-->
<script src="js/main.js"></script> <!-- Configurações padrões do java script -->
</body>

</html>